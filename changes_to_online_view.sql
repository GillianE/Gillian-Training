...
SELECT
	   ...
	   credit.third_party_aggregator,
	    ...
	    ELSE c.campaign_channel END AS campaign_channel,
	      ...
	      WHEN child.close_date::date <= '2019-04-01' AND credit.soft_credit_type_c IS NOT NULL AND credit.third_party_aggregator = 'Facebook' THEN 'Facebook'
              WHEN child.close_date::date <= '2019-04-01' AND credit.soft_credit_type_c IS NOT NULL AND credit.third_party_aggregator LIKE 'Actblue%' THEN 'ActBlue'
              WHEN child.close_date::date <= '2019-04-01' AND credit.soft_credit_type_c IS NOT NULL AND credit.third_party_aggregator = 'Charity Navigator' THEN 'Charity Navigator'
              WHEN child.close_date::date <= '2019-04-01' AND credit.soft_credit_type_c IS NOT NULL AND credit.third_party_aggregator = 'Crowdrise' THEN 'CrowdRise'
              WHEN child.close_date::date <= '2019-04-01' AND credit.soft_credit_type_c IS NOT NULL 
                    AND credit.third_party_aggregator NOT IN ('Facebook', 'Crowdrise', 'Charity Navigator') AND credit.third_party_aggregator NOT LIKE 'Actblue%' THEN 'Other'
  ...
  --                AND child.close_date BETWEEN '01-16-2017' AND '02-23-2018') THEN 'third_party_pre'
              WHEN (credit.soft_credit_type_c = 'Third Party'
                  AND child.r_c_giving_payment_method_c = 'Third Party Charge'
                  AND child.close_date >= '02-18-2018'
                  AND child.record_type_id  = '01236000000fBmOAAU'
		  AND child.deposit_site_c LIKE 'NA%') THEN 'third_party'
 ...
 FROM sf.opportunity AS parent
	...
	AND child.is_deleted = FALSE
     ...
     LEFT JOIN (SELECT c.r_c_giving_account_c
			, c.soft_credit_type_c
			, c.is_deleted
			, c.r_c_giving_opportunity_c
			, a.name AS third_party_aggregator
		FROM sf.r_c_giving_opportunity_credit_c c
		JOIN sf.v_account a
		ON c.r_c_giving_account_c = a.account_id) credit
...
--      OR
        ((credit.soft_credit_type_c = 'Third Party'
                  AND child.r_c_giving_payment_method_c = 'Third Party Charge'
                  AND child.close_date >= '02-18-2018'
                  AND child.record_type_id  = '01236000000fBmOAAU'
		  AND child.deposit_site_c LIKE 'NA%')
--      OR
